...
int CalculateDistortions::Init(PHCompositeNode *topNode)
{
  ...
  const int r_bins_N = 66;
  double r_bins[r_bins_N+1] = {217.83, 223.83, 229.83, 235.83, 241.83, 247.83, 253.83, 259.83, 265.83, 271.83, 277.83, 283.83, 289.83, 295.83, 301.83, 306.83, 311.05, 317.92, 323.31, 329.27, 334.63, 340.59, 345.95, 351.91, 357.27, 363.23, 368.59, 374.55, 379.91, 385.87,
...
int CalculateDistortions::process_event(PHCompositeNode *topNode)
{
  ...
  if (hits){
   ...
   //   z= forwardshift.Z();                            
        ...
        _isOnPlane = 0;
        if(!IsOverFrame(newPos.Perp()/mm,newPos.Phi())){
          _isOnPlane = 1;
        }
        _hit_z = z;
        _hit_r = r;
        ...
        for(int iz=0;iz<10;iz++){
          ...
          if( f_fill_ibf[iz]==1){
            ...
            _h_SC_ibf[iz] ->Fill(temp_phi,newPos.Perp()*cm,z_ibf[iz],_ibf_vol); //for now switching off the shifter code (first two are temp_phi, newPos.Perp()*cm)
          }
        }
        if( f_fill_ibf[0]==1){
          _h_R ->Fill(_hit_r);
